function [Models, kmg] = windowDiscriminantTrain2(Data, Y, pthresh, kmg, kmeanlimit, option)
...
%     kernelsz = mean(std(Data))
    ...
    %Scale this
    pscale = 1 / max(abs(probwindowd(:,3)));
    probwindowd(:,3) = probwindowd(:,3) .*pscale;


    %Only keep data if seperable wont work for more than binary
    subdata{1} = probwindowd(probwindowd(:,3) > pthresh ,[1,2]); %class 1
    subdata{2} = probwindowd(probwindowd(:,3) < -pthresh ,[1,2]); %class 2
...
%         hold on
        ...
        for mm = 1:km(cc) 
            ...
            hold on
            Ptemp = gaussianScaleArray(subdata{cc},Models{cc}.cluster{mm}.sigma,Models{cc}.cluster{mm}.mu);
            handle = Surface3D(subdata{cc}(:,1),subdata{cc}(:,2),Ptemp);
            hold on
        end
        cc = 2;
        for mm = 1:km(cc) 
