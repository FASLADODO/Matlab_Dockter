%make up some fake tissue parameters
...
for pp = 1:patients
    ...
    for gg = 1:length(key.cslist)
        ...
        for ss = 1:grasps
            ...
            %bit of noise)
            phi = paramsPatient{gg} + randn(1,key.paramsize).*paramNoise;
            ...
            PatientData{pp}.tissue{gg}.grasp{ss}.InputTrue = utrue;
            PatientData{pp}.tissue{gg}.grasp{ss}.StateTrue = statetrue;
            PatientData{pp}.tissue{gg}.grasp{ss}.Input = unoise;
            PatientData{pp}.tissue{gg}.grasp{ss}.State = statenoise;
            PatientData{pp}.tissue{gg}.grasp{ss}.Time = t;
            PatientData{pp}.tissue{gg}.grasp{ss}.Phi = phi;
...
save tissueData.mat AllData AllLabels ClassData ClassInput GraspData PatientData key input
