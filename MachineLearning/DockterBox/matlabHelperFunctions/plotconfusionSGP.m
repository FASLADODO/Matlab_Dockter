function out1 = plotconfusionSGP(varargin)
...
%% SGP Version
% in1 = varargin{1};
t =  nntype.data('format',varargin{1}) ;if iscell(t), t = cell2mat(t); end
y =  nntype.data('format',varargin{2}) ;if iscell(y), y = cell2mat(y); end
names = {''};
args = {t y names};
names = [0 ];
...
%               'color',bottomcolor); 
PTFS = 9;nnplots.title_font_size;
  trainColor = [0 0 1];
  valColor = [0 1 0];
  testColor = [1 0 0];
 ...
 % if (plotData.numSignals ~= numSignals) || (plotData.numClasses ~= numClasses)
    ...
    for plotrow=1:plotrows
      for plotcol=1:plotcols
        ...
        if (i<=numSignals)
          a = gca; %subplot(plotrows,plotcols,i);
          ...
          for j=1:numColumns
            ...
            if (j==numColumns) && (k==numColumns)
              c = nngui.blue*0+1;  % make white --tmk
              topcolor = [0 0.4 0];
              bottomcolor = [0.4 0 0];
              topbold = 'bold';
              bottombold = 'bold';
            elseif (j==k)
              c = nngui.green*0+1;  % make white --tmk
              topcolor = [0 0 0];
              bottomcolor = [0 0 0];
              topbold = 'bold';
              bottombold = 'normal';
            elseif (j<numColumns) && (k<numColumns)
              c = nngui.red*0+1;  % make white --tmk
              topcolor = [0 0 0];
              bottomcolor = [0 0 0];
              topbold = 'bold';
              bottombold = 'normal';
            elseif (j<numColumns)
              c = [0.5 0.5 0.5]*0+1;  % make white --tmk
              topcolor = [0 0.4 0];
              bottomcolor = [0.4 0 0];
              topbold = 'normal';
              bottombold = 'normal';
    ...
    for j=1:numColumns
      for k=1:numColumns
        ...
        if ~isempty(bottom)
            ...
            set(axisdata.number(j,k),'string',top,'verticalalignment', 'middle');
        end
      end
    end
  end
