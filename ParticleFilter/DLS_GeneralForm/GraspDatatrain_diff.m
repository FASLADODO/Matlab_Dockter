...
figure(1)
for jj = 1:num_Hard_Grasps
    h1 = quiver(Data_Hard(jj).strain,Data_Hard(jj).strainDot,Data_Hard(jj).strainDot,Data_Hard(jj).strainDotDot,'g');
    hold on
end

for jj = 1:num_Soft_Grasps
    h2 = quiver(Data_Soft(jj).strain,Data_Soft(jj).strainDot,Data_Soft(jj).strainDot,Data_Soft(jj).strainDotDot,'r');
    hold on
end
hold off
...
temp = [];
for ii = 1:length(Error_CumSum_Hard)
    %Check over time if error1 was less than error2
    temp = [temp; Error_CumSum_Hard{ii}(:,1) < Error_CumSum_Hard{ii}(:,2)];
end

timeclassify_hard = sum(temp)/length(temp); %percent of correct over time
...
temp = [];
for ii = 1:length(Error_CumSum_Soft)
    %Check over time if error2 was less than error 1
    temp = [temp; Error_CumSum_Soft{ii}(:,2) < Error_CumSum_Soft{ii}(:,1)];
end

timeclassify_soft = sum(temp)/length(temp); %percent of correct over time
