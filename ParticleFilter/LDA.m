...
for i = 1:k,
    ...
    Group      = (Target == ClassLabel(i)); %(=1 for each row that corresponds to a particular group, else =0)
    nGroup(i)  = sum(double(Group)); %total number of entrys for each group
    
    % Calculate group mean vectors
    GroupMean(i,:) = mean(Input(Group,:)); % takes average of all entries for a particular group
